...
for (s in 1:reps){
  ...
  check = -1000
  while(check == -1000){
    random_start <- c(current_picks, sample(available, 12 - (current_week-1)))
  ...
  for (i in 1:iters){
    ...
    if (swap_replace == 1){
      # replace  1 
      new_week <- sample(current_week:12,1)
      new_team <- sample(c(1:32)[!(1:32 %in% current_pick)], 1)
      
      new_pick[new_week] <- new_team
    } else {
      # swap
      swap_weeks <- sample(current_week:12, 2, replace = F)
...
write_csv(ranked, paste0("Data2023/results_weighted_Week",current_week, ".csv"), append = T)
